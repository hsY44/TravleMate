                ...
                <div class="flex items-center gap-3">
                    <a href="${ctx}/admin/qnas"
                       class="flex items-center gap-1 text-sm text-slate-400 hover:text-slate-600 transition">
                        <i data-lucide="arrow-left" class="h-4 w-4"></i> 문의 목록
                    </a>
                    <h1 class="text-2xl font-bold text-slate-900">문의 카테고리 관리</h1>
                </div>
                <button onclick="openAddModal()"
                        class="flex items-center gap-1.5 rounded-xl bg-sky-500 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-600">
                    <i data-lucide="plus" class="h-4 w-4"></i> 카테고리 추가
                </button>
            </div>
            ...
            <div class="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
                <table class="w-full text-sm text-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <c:forEach var="t" items="${qnaTypes}">
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-3 font-mono text-slate-400">${t.qnaTypeCd}</td>
                                <td class="px-4 py-3 font-semibold text-slate-900">${t.qnaTypeNm}</td>
                                <td class="px-4 py-3">
                                    <button onclick="openEditModal('${t.qnaTypeCd}', '${t.qnaTypeNm}')"
                        ...
                        <c:if test="${empty qnaTypes}">
                            <tr>
                                <td colspan="3" class="py-12 text-center text-slate-400">등록된 카테고리가 없습니다.</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
...
<!-- 등록/수정 모달 -->
<div id="typeModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40"
     onclick="closeModal()">
    <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onclick="event.stopPropagation()">
        <div class="mb-4 flex items-center justify-between">
            <h3 id="modalTitle" class="text-lg font-semibold text-slate-900">카테고리 추가</h3>
        ...
        <div class="flex flex-col gap-3">
            <div id="cdField">
                ...
                <input type="text" id="qnaTypeCd" placeholder="예: QNA001"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400">
            </div>
            <div>
                <label class="mb-1 block text-xs font-semibold text-slate-500">카테고리명</label>
                <input type="text" id="qnaTypeNm" placeholder="카테고리명을 입력하세요"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400">
            </div>
        </div>
        <p id="typeModalMsg" class="mt-2 hidden text-xs text-red-400"></p>
...
function openAddModal()
{
    ...
    document.getElementById('qnaTypeCd').value         = '';
    document.getElementById('qnaTypeNm').value         = '';
    document.getElementById('cdField').style.display   = '';
    document.getElementById('typeModalMsg').classList.add('hidden');
    showModal();
}
...
function openEditModal(cd, nm)
{
    ...
    document.getElementById('qnaTypeNm').value         = nm;
...
function saveType()
{
    const cd  = editMode ? document.getElementById('editCd').value : document.getElementById('qnaTypeCd').value.trim();
    const nm  = document.getElementById('qnaTypeNm').value.trim();
    ...
    const url    = editMode ? ctx + '/admin/qnaType/' + cd : ctx + '/admin/qnaType';
    const method = editMode ? 'PUT' : 'POST';

    fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ qnaTypeCd: cd, qnaTypeNm: nm })
    })
    .then(r => r.json())
    .then(data => {
