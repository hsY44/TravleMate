...
<head>
    ...
    <title>트래블메이트 — FAQ 카테고리 관리</title>
                ...
                <div class="flex items-center gap-3">
                    <a href="${ctx}/admin/faqs"
                       class="flex items-center gap-1 text-sm text-slate-400 hover:text-slate-600 transition">
                        <i data-lucide="arrow-left" class="h-4 w-4"></i> FAQ 목록
                    </a>
                    <h1 class="text-2xl font-bold text-slate-900">FAQ 카테고리 관리</h1>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <c:forEach var="t" items="${faqTypes}">
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3">
                                    <button onclick="openEditModal('${t.faqTypeCd}', '${t.faqTypeNm}')"
    ...
    <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onclick="event.stopPropagation()">
        ...
        <div class="flex flex-col gap-3">
            <div id="cdField">
                ...
                <input type="text" id="faqTypeCd" placeholder="예: FAQ001"
            ...
            <div>
                ...
                <input type="text" id="faqTypeNm" placeholder="카테고리명을 입력하세요"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400">
            </div>
        </div>
...
function openAddModal()
{
    ...
    document.getElementById('faqTypeCd').value         = '';
    document.getElementById('faqTypeNm').value         = '';
...
function openEditModal(cd, nm)
{
    ...
    document.getElementById('faqTypeNm').value         = nm;
...
function saveType()
{
    const cd  = editMode ? document.getElementById('editCd').value : document.getElementById('faqTypeCd').value.trim();
    const nm  = document.getElementById('faqTypeNm').value.trim();
    ...
    const url    = editMode ? ctx + '/admin/faqType/' + cd : ctx + '/admin/faqType';
    const method = editMode ? 'PUT' : 'POST';

    fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ faqTypeCd: cd, faqTypeNm: nm })
    })
    .then(r => r.json())
    .then(data => {
